...
def _lerobot_pin(text: str) -> re.Match[str]:
    """The consistent lerobot git pin in `text`, or a useful failure.
    ...
    assert matches, "expected at least one lerobot git pin"
    refs = {(match["url"], match["sha"]) for match in matches}
    assert len(refs) == 1, f"lerobot git pins disagree: {sorted(refs)}"
    return matches[0]
